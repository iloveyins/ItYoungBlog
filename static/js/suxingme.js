...
jQuery(document).ready(function($) {
    ...
    $.fn.postLike = function() {
       var id = $(this).data("id"),
        ...
        if ($(this).hasClass('current')) {
          ...
          $.post('index.php?c=Index&a=LikeArticleMinus',{id:id},function(ret){
           rateHolder.text(Number(rateHolder.text()) - 1);
          },"JSON");
            return false;
        } else {
            $(this).addClass('current');
            $.post('index.php?c=Index&a=LikeArticleAdd',{id:id},function(ret){
             rateHolder.text(Number(rateHolder.text()) + 1);
            },"JSON");
            return false;
        }
    ...
    */
    ...
    !function(o){"use strict";o.fn.toTop=function(t){var i=this,e=o(window),
      s=o("html, body"),n=o.extend({autohide:!0,offset:420,speed:500,position:!0,right:38,bottom:45},t);
      ...
      e.scroll(function(){var o=e.scrollTop();n.autohide&&(o>n.offset?i.fadeIn(n.speed):i.fadeOut(n.speed))})}}(jQuery);
    $(function() {
        $('.to-top').toTop();
     });
    $('body').append('<a class="to-top"><i class="fa fa-level-up" aria-hidden="true"></i></a>');
...
jQuery(document).on("click", "#fa-loadmore", function($) {
    ...
    if (_self.hasClass('is-loading')) {
        ...
        _button.html('<i class="icon-spin6 animate-spin"></i> 鍔犺浇涓�...');
        ...
        jQuery.ajax({
            ...
            success: function(data) {
                if (data.code == 500) {
                    _button.data("paged", data.next).html('鍔犺浇鏇村');
                    createButterbar('鏈嶅姟鍣ㄦ鍦ㄥ姫鍔涙壘鍥炶嚜鎴�  o(鈭鈭�)o')
...
jQuery(document).on("click", ".post-nav span", function($) {
    ...
    if (_self.hasClass('is-loading')) {
        ...
        _postlistWrap.html('<div class="wait-tips"><i class="icon-spin6 animate-spin"></i> 鍔犺浇涓�...</div>');
        _self.addClass('is-loading');
...
jQuery(document).on("click", "#nice-check-contribute", function($) {
    ...
    if( !title ){
        createButterbar('鏂囩珷鏍囬涓嶈兘涓虹┖锛�');
        return false;
    }

    if( !post_content ){
        createButterbar('鏂囩珷鍐呭涓嶈兘涓虹┖锛�');
        return false;
    }

    if( !cs ){
        createButterbar('鑷冲皯閫夋嫨涓€涓垎绫伙紒');
        return false;
    }

    if( type == 1 && ( name == '' || source =='' ) ){
        createButterbar('璇峰～鍐欏畬鏁寸増鏉冭鏄�');
        return false;
    }
        ...
        success: function(data) {
            if( data.code == 200 ){
                ...
                function CountDown() {
                    ...
                    jQuery("#nice-do-contribute-verify-code").html( count + " 绉掗噸鍙戦偖浠�");
                    if (count == 0) {
                        jQuery("#nice-do-contribute-verify-code").html("閲嶅彂閭欢").removeAttr("disabled");
        ...
        jQuery("#upimg").change(function(){
            if( jQuery("#upimg").val() != ''){
                jQuery("#imgupload").ajaxSubmit({
                    ...
                    error:function(xhr){
                        createButterbar('缃戠粶閿欒锛岃绋嶅悗鍐嶈瘯锛�');
                    }
                });
            }
        });
...
jQuery('#do-contribute').click(function() {
    ...
    if( email == '' ){
        createButterbar('璇峰～鍐欓偖绠�');
        return false;
    }

    if( code == '' ){
        createButterbar('璇峰～鍐欓獙璇佺爜');
        return false;
    }
